...
class ShippingControllerKstNormalizationNotDoneTest {
    ...
    @Test
    @DisplayName("GET /shipping/not-done: range 우선 및 역전 스왑 확인")
    void notDone_rangeWins_andSwap() throws Exception {
        when(shippingService.getNotDone(anyString(), anyString(), anyString(), any())).thenReturn(List.of());
    ...
    void notDone_singleDate_callsSingleOverload() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/shipping/not-done")
                ...
                .andExpect(jsonPath("$.data.items[0].noteId", is(30)));
    ...
    @Test
    @DisplayName("GET /shipping/not-done: dateFrom만 있을 때 date=null로 범위 오버로드 호출(from만 설정, to=null)")
    ...
    @Test
    @DisplayName("GET /shipping/not-done: dateTo만 있을 때 date=null로 범위 오버로드 호출(to만 설정, from=null)")
    void notDone_dateToOnly_callsRangeOverload_withNullDate() throws Exception {
        ...
        verify(shippingService).getNotDone(dateCaptor2.capture(), fromCaptor2.capture(), toCaptor2.capture(), wcCaptor2.capture());
