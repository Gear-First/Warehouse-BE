...
class ShippingServiceWarehouseTest {
    ...
    void complete_decreasesOnlyMatchingWarehouse() {
        ...
        inventory.increase(1L, 1001L, 10);
        inventory.increase(2L, 1001L, 10);

        // when: complete
        var resp = service.complete(8001L);

        // then: WH1 becomes 0, WH2 remains 10
        var wh1 = inventory.listOnHand(1L, null, 0, 100).getItems().stream()
                .filter(i -> i.part().id().equals(1001L))
                .findFirst().orElseThrow();
        assertEquals(0, wh1.onHandQty());

        var wh2 = inventory.listOnHand(2L, null, 0, 100).getItems().stream()
                .filter(i -> i.part().id().equals(1001L))
                .findFirst().orElseThrow();
    ...
    void complete_conflict_whenInsufficientInWarehouse() {
        ...
        inventory.increase(5L, 2001L, 5); // insufficient
    ...
    private record InventoryBackedOnHandProvider(InventoryService inventory)
        ...
        @Override public int getOnHandQty(Long productId) {
            ...
            return inventory.listOnHand(null, null, 0, 100).getItems().stream()
    ...
    private static class FakeInventoryService implements InventoryService {
        private static final class Key {
            final Long wh; final Long part;
            Key(Long wh, Long part){ this.wh = wh; this.part = part; }
        ...
        public PageEnvelope<OnHandSummary> listOnHand(Long warehouseId, String keyword, int page, int size) {
            ...
            var list = new java.util.ArrayList<OnHandSummary>();
            for (var e : map.entrySet()) {
                var key = e.getKey();
                if (warehouseId != null && !java.util.Objects.equals(warehouseId, key.wh)) continue;
        ...
        public void increase(Long warehouseId, Long partId, int qty) {
            if (qty<=0||partId==null) return; var k=new Key(warehouseId,partId); map.merge(k, qty, Integer::sum);
        }
        @Override
        public void decrease(Long warehouseId, Long partId, int qty) {
            if (qty<=0||partId==null) return; var k=new Key(warehouseId,partId);
