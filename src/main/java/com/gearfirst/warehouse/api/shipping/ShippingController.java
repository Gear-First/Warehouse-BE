...
public class ShippingController {
    ...
    @Operation(summary = "출고 항목 업데이트", description = "출고 내역서의 특정 항목에 대해 집품 수량(pickedQty)만 업데이트합니다. 상태는 서버가 도출합니다.")
    ...
    @Operation(summary = "출고 통합 리스트 조회", description = "상태 파라미터로 not-done|done|all을 선택하여 조회합니다. 날짜/창고 필터링 지원. 기본 정렬: noteId asc (phase-1: date/dateFrom/dateTo는 requestedAt에 적용)")
    @GetMapping("/notes")
    public ResponseEntity<ApiResponse<PageEnvelope<ShippingNoteSummaryResponse>>> getNotes(
